...
import org.openmrs.module.southsudanemr.builder.RegisterPatientFormBuilder;
import org.openmrs.module.southsudanemr.utils.SsRegistrationAppUiUtils;
...
public class EditSectionPageController {
	...
	public void get(UiSessionContext sessionContext, PageModel model, @RequestParam("patientId") Patient patient,
		...
		addModelAttributes(model, patient, formStructure.getSections().get(sectionId), administrationService, returnUrl, app);
	}
	...
	public String post(UiSessionContext sessionContext, PageModel model,
	        ...
	        @BindParams PersonName name, @RequestParam(value = "birthdateYears", required = false) Integer birthdateYears,
	        @RequestParam(value = "birthdateMonths", required = false) Integer birthdateMonths,
	        @RequestParam("appId") AppDescriptor app, @RequestParam("sectionId") String sectionId,
	        @RequestParam("returnUrl") String returnUrl, @SpringBean("patientService") PatientService patientService,
		...
		SsRegistrationAppUiUtils.validateLatitudeAndLongitudeIfNecessary(address, errors);
		...
		if (formStructure != null) {
			...
			try {
				...
				SsRegistrationAppUiUtils.checkForIdentifierExceptions(ex, errors);
			}
			...
			try {
				...
				if (sectionId.equals("contactInfo")) {
					InfoErrorMessageUtil.flashInfoMessage(
					    request.getSession(),
					    ui.message("registrationapp.editContactInfoMessage.success",
					...
					InfoErrorMessageUtil.flashInfoMessage(request.getSession(), ui.message(
					    "registrationapp.editCustomSectionInfoMessage.success",
					    patient.getPersonName() != null ? ui.encodeHtml(patient.getPersonName().toString()) : "",
					    sectionLabel));
				}
	...
	private void addModelAttributes(PageModel model, Patient patient, Section section, AdministrationService adminService,
	        String returnUrl, AppDescriptor app) throws Exception {
		...
		model.put("uiUtils", new SsRegistrationAppUiUtils());
