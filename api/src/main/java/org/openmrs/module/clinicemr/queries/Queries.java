package org.openmrs.module.clinicemr.queries;
...
public class Queries {
	...
	public static final String getAppointmentToday(int conceptId) {
		return "SELECT pa.patient_id FROM patient pa INNER JOIN encounter e ON pa.patient_id=e.patient_id"
		        ...
		        + ClinicEmrReportUtils.formatDate(ClinicEmrReportUtils.getStartOfDay()) + "' AND '"
		        + ClinicEmrReportUtils.formatDate(ClinicEmrReportUtils.getEndOfDay()) + "'";
	}
	...
	public static final String getPatientsWhoMissedAppointment(int conceptId) {
		return "SELECT patient_id FROM "
		        ...
		        + conceptId + " AND DATEDIFF('" + ClinicEmrReportUtils.formatDate(ClinicEmrReportUtils.getStartOfDay())
		        + "' ,o.value_datetime) > 0";
	}
}
