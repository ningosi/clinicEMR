 ...
 */
package org.openmrs.module.clinicemr.utils;
...
public class ClinicRegistrationAppUiUtils {
	...
	/**
	 * Gets the patient identifier value for the specified person for the identifierTypeUuid that
	...
	public String getIdentifier(Patient patient, String identifierTypeUuid) {
		if (patient != null) {
			PatientIdentifier patientIdentifier = patient.getPatientIdentifier(Context.getPatientService()
	...
	public static void validateLatitudeAndLongitudeIfNecessary(PersonAddress address, BindingResult errors) {
		if (address != null) {
			...
			Map<String, String> regex = Context.getRegisteredComponent(AddressSupportCompatibility.ID,
			    AddressSupportCompatibility.class).getElementRegex();
			
			if (StringUtils.isNotBlank(address.getLatitude())) {
				if (regex != null && StringUtils.isBlank(regex.get("latitude"))) {
					if (!ClinicRegistrationAppUiUtils.isValidLatitude(address.getLatitude())) {
			...
			if (StringUtils.isNotBlank(address.getLongitude())) {
				if (regex != null && StringUtils.isBlank(regex.get("longitude"))) {
					if (!ClinicRegistrationAppUiUtils.isValidLongitude(address.getLongitude())) {
		...
		if (ex instanceof PatientIdentifierException) {
			if (ex instanceof InvalidCheckDigitException) {
				...
				errors.reject("registrationapp.error.identifier.duplicate",
				    Collections.singleton(((DuplicateIdentifierException) ex).getPatientIdentifier()).toArray(), null);
			} else if (ex instanceof IdentifierNotUniqueException) {
				errors.reject("registrationapp.error.identifier.duplicate",
				    Collections.singleton(((IdentifierNotUniqueException) ex).getPatientIdentifier()).toArray(), null);
			} else {
